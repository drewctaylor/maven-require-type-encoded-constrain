...
public final class JavaPoetUtility
{
    ...
    // {
    // final ClassName className = className(parameterCount, exception, effect);
    //
    // final Function<TypeSpec.Builder, TypeSpec.Builder> typeSpecBuilderAddMethodByEffect = builder -> effect ?
    // builder :
    // builder
    // .addMethod(then(parameterCount, exception))
    // .addMethods(c(parameterCount, exception).collect(toList()));
    //
    // final Function<TypeSpec.Builder, TypeSpec.Builder> typeSpecBuilderAddMethodByException = builder -> exception ?
    // builder.addMethod(e(parameterCount, effect)) :
    // builder;
    //
    ...
    // effect ?
    // builder
    // .addMethods(c(parameterCount, exception, effect).collect(toList())) :
    // builder :
    // parameterCount == 1 ?
    // builder
    // .addMethods(c(parameterCount, exception, effect).collect(toList()))
    // .addMethod(apply(parameterCount, exception, effect)) :
    // builder
    // .addMethods(c(parameterCount, exception, effect).collect(toList()))
    // .addMethod(apply(parameterCount, exception, effect))
    // .addMethod(reverse(parameterCount, exception, effect))
    // .addMethod(curry(parameterCount, exception, effect))
    // .addMethod(uncurry(parameterCount, exception, effect));
    //
    // final String javadoc = concat(
    // of(format("A %s-parameter %s%s.", parameterCount, effect ? "effect" : "function", exception ? " that may throw an
    // exception" : "")),
    // typeVariableNameJavadocList(0, parameterCount, typeVariableNameR(1), exception, effect))
    ...
    // .addSuperinterfaces(superInterface(parameterCount, exception, effect).collect(toList()))
    // .addMethod(f(parameterCount, exception, effect))
    // .addMethod(fN(parameterCount, exception, effect)))))
    // .addMethods(methodSpec(parameterCount, exception, effect).collect(toList()))
    // .build())
    ...
    // }
    //
    // private static Stream<TypeName> superInterface(
    // final int parameterCount,
    // final boolean exception,
    // final boolean effect)
    // {
    ...
    // effect ?
    // parameterCount == 1 ?
    // of(ParameterizedTypeName.get(ClassName.get("java.util.function", "Consumer"), typeVariableNameList(0, parameterCount,
    // typeVariableNameR(1), exception, effect).toArray(TypeVariableName[]::new))) :
    // parameterCount == 2 ?
    // of(ParameterizedTypeName.get(ClassName.get("java.util.function", "BiConsumer"), typeVariableNameList(0,
    // parameterCount, typeVariableNameR(1), exception, effect).toArray(TypeVariableName[]::new))) :
    // empty() :
    // parameterCount == 0 ?
    // of(ParameterizedTypeName.get(ClassName.get("java.util.function", "Supplier"), typeVariableNameList(0, parameterCount,
    // typeVariableNameR(1), exception, effect).toArray(TypeVariableName[]::new))) :
    // parameterCount == 1 ?
    // of(ParameterizedTypeName.get(ClassName.get("java.util.function", "Function"), typeVariableNameList(0, parameterCount,
    // typeVariableNameR(1), exception, effect).toArray(TypeVariableName[]::new))) :
    // parameterCount == 2 ?
    // of(ParameterizedTypeName.get(ClassName.get("java.util.function", "BiFunction"), typeVariableNameList(0,
    // parameterCount, typeVariableNameR(1), exception, effect).toArray(TypeVariableName[]::new))) :
    // empty();
    // }
    //
    // private static Stream<MethodSpec> methodSpec(
    // final int parameterCount,
    // final boolean exception,
    // final boolean effect)
    // {
    ...
    // .addParameters(parameterSpecList(parameterCount).collect(toList()));
    //
    // final var builder2 = exception ? builder1.addException(e) : builder1;
    // final var builder3 = effect ? builder2 : builder2.returns(r);
    //
    ...
    // final var r2 = typeVariableNameR(2);
    // final var parameterSpec = parameterSpec(ParameterizedTypeName.get(className(1, false, false), r1, r2));
    //
    // final var javadocReturn = "a function that is the given function composed with this function";
    //
    ...
    // .addModifiers(PUBLIC, DEFAULT)
    // .addTypeVariable(r2)
    // .returns(parameterizedTypeName(parameterCount, r2, exception, false))
    // .addParameter(parameterSpec)
    // .addStatement("$T.requireNonNull($N, \"$N\")", Require.class, parameterSpec.name, parameterSpec.name)
    // .addStatement(format("return (%s) -> $N.f(f(%s))", parameterNameListDelimitedByComma(parameterCount),
    // parameterNameListDelimitedByComma(parameterCount)), parameterSpec.name)
    // .build();
    // }
    //
    // /**
    ...
    // final boolean effect)
    // {
    // final var p1 = TypeVariableName.get("P1");
    //
    // final var javadocReturn = effect ? "an effect that does nothing" : "a function that returns its first parameter value
    // as its output value";
    //
    // final var javadoc = concat(concat(
    ...
    // .addStatement(format(effect ? "return (%s) -> {}" : "return (%s) -> p1",
    ...
    // final boolean exception)
    // {
    // final var r = typeVariableNameR(1);
    // final var parameterSpec = parameterSpec(r);
    //
    // final var javadocReturn = "a function that returns the given value as its output value";
    //
    // final var javadoc = concat(concat(of(
    // format("Returns %s.", javadocReturn),
    // format("@param %s the given value", parameterSpec.name)),
    // typeVariableNameJavadocList(0, parameterCount, r, exception, false)),
    // of(format("@return %s", javadocReturn)))
    // .collect(joining(lineSeparator()));
    //
    ...
    // .addJavadoc(javadoc)
    // .addModifiers(PUBLIC, STATIC)
    // .addTypeVariables(typeVariableNameList(0, parameterCount, r, exception, false).collect(toList()))
    // .returns(parameterizedTypeName(parameterCount, r, exception, false))
    // .addParameter(parameterSpec)
    // .addStatement(format("return (%s) -> $N", parameterNameListDelimitedByComma(parameterCount)), parameterSpec.name)
    // .build();
    //
    // return concat(
    // of(methodSpecF.apply(MethodSpec
    // .methodBuilder("c"))),
    // !exception && parameterCount == 1 ?
    // of(methodSpecF.apply(MethodSpec
    ...
    // final boolean effect)
    // {
    // final var e = typeVariableNameE();
    // final var r = typeVariableNameR(1);
    // final var parameterSpec = parameterSpec(e);
    //
    ...
    // .addParameter(parameterSpec)
    // .addStatement("$T.requireNonNull($N, \"$N\")", Require.class, parameterSpec.name, parameterSpec.name)
    // .addStatement(format("return (%s) -> { throw $N; }", parameterNameListDelimitedByComma(parameterCount)),
    // parameterSpec.name)
    // .build();
    // }
    //
    ...
    // {
    // final var r = typeVariableNameR(1);
    // final var parameterSpec = parameterSpec(typeVariableNameF(0, parameterCount, r, exception, effect));
    //
    // final var javadocReturn = format("a form of the given %s that is of this %s type", effect ? "effect" : "function",
    ...
    // final var p1 = TypeVariableName.get("P1");
    //
    // final var javadocReturn = format("a form of this %s with the value of parameter 1 fixed to the given value", effect ?
    // "effect" : "function");
    //
    // final var javadoc = of(
    // format("Returns %s.", javadocReturn),
    ...
    // .addStatement(format("return (%s) -> f(%s)", parameterNameListDelimitedByCommaShift(parameterCount),
    // parameterNameListDelimitedByComma(parameterCount)))
    // .build();
    // }
    //
    ...
    // final boolean effect)
    // {
    // final var javadocReturn = format("a form of this %s where the order of the parameters is reversed", effect ? "effect"
    // : "function");
    //
    // final var javadoc = of(
    // format("Returns %s.", javadocReturn),
    ...
    // .addStatement(format("return (%s) -> f(%s)", parameterNameListDelimitedByCommaReverse(parameterCount),
    // parameterNameListDelimitedByComma(parameterCount)))
    // .build();
    // }
    //
    ...
    //
    // final var beforeOpen = typeName.toString().contains("<") ? typeName.toString().substring(0,
    // typeName.toString().indexOf('<')) : typeName.toString();
    // final var afterPackage = beforeOpen.contains(".") ? beforeOpen.substring(beforeOpen.lastIndexOf('.') + 1) :
    // beforeOpen;
    // final var parameterName = afterPackage.toLowerCase();
    //
...
//     * // @formatter:on
    ...
    // /**
    // * Return a parameterized type name for the given parameter count, return type variable name, exception, and effect;
    // in this case, reverse the parameter types.
    // * <p>
//     * // @formatter:off
//     * <ul>
...
//     * // @formatter:on
    ...
    // requireZeroOrPositive(parameterIndex, "parameterIndex");
    //
    // return format("@param %s the value of parameter %s", parameterSpec(typeVariableNameP(parameterIndex)).name,
    // parameterIndex + 1);
    // }
    //
    // /**
...
//     * // @formatter:on
    ...
    //
    // /**
    // * Return a javadoc param list for the given parameter index, parameter count, return type variable name, exception,
    // and effect.
    // * <p>
//     * // @formatter:off
//     * <ul>
...
//     * // @formatter:on
    ...
    // {
    // final var reverse = stream.collect(toList());
    ...
    // {
    // final var list = stream.collect(toList());
    //
    // return list.isEmpty() ?
    // supplier.get() :
    ...
    // .addParameters(fieldSpecList.stream().map(fieldSpec -> ParameterSpec.builder(fieldSpec.type, fieldSpec.name,
    // FINAL).build()).collect(toList()))
    // .addCode(fieldSpecList.stream().map(fieldSpec -> CodeBlock.of("this.$N = $N;", fieldSpec.name,
    // fieldSpec.name)).collect(CodeBlock.joining("")))
    // .build();
    // }
    //
    ...
    // of(returnJavadoc(CodeBlock.of(format("return %s", javadoc.apply(fieldSpecList.get(fieldSpecIndex),
    // fieldSpecIndex))))))
    ...
    // .addParameters(fieldSpecList.stream().map(fieldSpec -> ParameterSpec.builder(fieldSpec.type, fieldSpec.name,
    // FINAL).build()).collect(toList()))
    // .addCode(CodeBlock.of("return new $N<>($L);", "Default", fieldSpecList.stream().map(fieldSpec ->
    // CodeBlock.of(fieldSpec.name)).collect(CodeBlock.joining(","))));
    // }
    //
